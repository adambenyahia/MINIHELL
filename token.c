...
/*   Updated: 2023/01/07 00:24:46 by beadam           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_token(t_tokens **head, char *line)
{
	...
	{
		if (*line == '|')
			i = set_token(head, line, 1, PIPE); // pipe
		else if (*line == ' ')
			i = set_token(head, line, 1, SP); // SPACE
		else if (*line == '\'')
			i = get_single_quote(head, line); // SQUOTE
		else if (*line == '"')
			i = get_double_quote(head, line); // DQUOTE
		else if (*line == '>' && *(line + 1) != '>')
			...
			i = set_token(head, line, 2, HERDOC); // <<
					...
					|| *(line + 1) == '?' || *(line + 1) == '0' || *(line
						+ 1) == '$'))
			i = tokenize_variables(head, line); // variables
		else
			i = set_word_token(head, line); // WORD
		line += i;
	}
}
